      ...
      real *8 gradr(2),gradl(2),gradex(2)
...
c
      ...
      if(alpha(icint).lt.-pi) alpha(icint) = alpha(icint)+2*pi


      rpan = 0.3d0
      thet = alpha(icint)

      call getcornermat(zk,thet,ncorner,rpan,ts,wts,xmatc(1,1,icint))
...
c
      ...
      rab = rmus(iregr(iedge))*rnus(iregl(iedge)) - 
     1      rmus(iregl(iedge))*rnus(iregr(iedge))   

      dab = rmus(iregr(iedge))*rnus(iregl(iedge)) + 
     1      rmus(iregl(iedge))*rnus(iregr(iedge))   
      ...
      xmat(i,i) =-0.5d0/rgab*rsgn(iedge)

      call getrhs3(iregr(iedge),ncharges,xsrc,ysrc,charges,xs(i),ys(i),
     1      potr,gradr)
      call getrhs3(iregl(iedge),ncharges,xsrc,ysrc,charges,xs(i),ys(i),
     1      potl,gradl)


      dudnr = gradr(1)*rnxe(iedge) + gradr(2)*rnye(iedge)
      dudnl = gradl(1)*rnxe(iedge) + gradl(2)*rnye(iedge)

      rhs(i) = rmus(iregr(iedge))*potr - rmus(iregl(iedge))*potl
      rhs(i) = rhs(i)*sqrt(qwts(i))/rab*rnus(iregl(iedge))*
     1          rnus(iregr(iedge))*diags(i) 

      rhsneu(i) = rnus(iregr(iedge))*dudnr - rnus(iregl(iedge))*dudnl
      rhsneu(i) = -rhsneu(i)*sqrt(qwts(i))/rab*rmus(iregl(iedge))*
     1          rmus(iregr(iedge))*diags(i) 
...
c
cc      rescale xmat to make sure it is the right scaling
c
      do i=1,n
      do j=1,n

        xmat(i,j) = xmat(i,j)*diags(i)
...
c
      ...
      ireg = 1
      call getrhs3(ireg,ncharges,xsrc,ysrc,charges,trg(1),trg(2),
     1    potex,gradex)

      pot = 0

      call comppot2(trg,n,xs,ys,rnx,rny,qwts,soln,solnneu,potd,pots)

      pot = potd/rnus(1) + pots/rmus(1)
      call prin2('pots=*',pots,1)
      call prin2('potd=*',potd,1)
...
c---------------------------------------------
       ...
       subroutine getcornermat(zk,thet,n,rlen,ts,wts,xmat)
       implicit real *8 (a-h,o-z)
       real *8 ts(*),wts(*),rr
       complex *16 zk,z,h0,h1,xmat(n,*),ima,ima4inv
...
c
c          an extra rlen may be required - check this?
c
         xmat(i,j) = -ima4inv*h1/rr*zk*sint*ts(j)*rlen*
     1       sqrt(wts(i)*wts(j)) 
...
c--------------------------------------------
      ...
      subroutine getrhs(ireg,xsrc,ysrc,charges,x,y,pot,grad)
      implicit real *8 (a-h,o-z)
      ...
      pot = log(rrr)*charges(ireg)
      grad(1) = dx/rrr**2*charges(ireg)
      grad(2) = dy/rrr**2*charges(ireg)
...
c------------------------------------       
      ...
      subroutine getrhs2(ireg,pars,x,y,pot)
      implicit real *8 (a-h,o-z)
      real *8 pars(*) 
...
c------------------------------------       
      ...
      subroutine getrhs3(ireg,nc,xsrc,ysrc,charges,x,y,pot,grad)
      implicit real *8 (a-h,o-z)
      real *8 xsrc(*),ysrc(*),charges(*),grad(2)
