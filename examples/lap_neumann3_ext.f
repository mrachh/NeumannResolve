     ...
     1   rkappa2(:)
      ...
      real *8, allocatable :: work_ref(:),wr_ref(:),wi_ref(:)
      real *8, allocatable :: work2(:),wr2(:),wi2(:)
      ...
      real *8, allocatable :: xmat_ref_eig(:,:),xmat2_eig(:,:)
...
c
c      Exterior neumann problem Laplace
c
...
c
     ...
     1    1,'a*')
      do ie=1,nedges
        istart = lns(ie)
        do j=1,ncorner
          write(96,*) xs(istart+j-1),ys(istart+j-1)
        enddo
     ...
     1    1,'a*')
      do ie=1,nedges
        istart = lns2(ie)
        do j=1,ncorner2
          write(97,*) xs2(istart+j-1),ys2(istart+j-1)
        enddo
...
c
     ...
     1          xmatc_ref(1,1,icint))
      ...
      allocate(xmat_ref_eig(nref,nref))
      allocate(xmat2_eig(n2,n2))
     ...
     1       grad(2)*rnye(iedge))
      ...
      do i=1,n2
        do j=1,n2
          ...
          xmat2_eig(j,i) = xmat2(j,i)
        enddo
      enddo

      do i=1,nref
        do j=1,nref
          xmat_ref_eig(j,i) = xmat_ref(j,i)
        enddo
      enddo

      lwork_ref = 10*nref
      allocate(work_ref(lwork_ref))
      allocate(wr_ref(nref),wi_ref(nref))

      call dgees('N','N',sel0,nref,xmat_ref_eig,nref,sdim,wr_ref,wi_ref,
     1       vs_ref,1,work_ref,lwork_ref,bwork,info)
      call prinf('info=*',info,1)
 
      open(unit=77,file='ref-eigs.dat')
      do i=1,nref
        write(77,*) wr_ref(i),wi_ref(i)
      enddo
      close(77)

      lwork2 = 10*n2
      allocate(work2(lwork2))
      allocate(wr2(n2),wi2(n2))

      call dgees('N','N',sel0,n2,xmat2_eig,n2,sdim,wr2,wi2,
     1       vs2,1,work2,lwork2,bwork,info)
      call prinf('info=*',info,1)
      open(unit=77,file='sp-eigs.dat')
      do i=1,n2
        write(77,*) wr2(i),wi2(i)
      enddo
      close(77)
...
C$      t2 = omp_get_wtime()     
 ...
 1467 format(3(2x,e22.16))      
     ...
     1     pottarg_rand(i)
      ...
      write(fres,'(a,i3.3,a,i2.2,a)') "ext_res_",nlat,"_",nlev,".dat"
...
c
      ...
      alpha = 1.0d0
      beta = 0
      call dgemv('n',nc0,nc0,alpha,vmat,nc0,vals,1,beta,coefs,1)
...
c
      ...
      do i=1,ncorner
        do j=1,ncorner
          xmatsub(i,j+ncorner) = xmat2copy(i+istart,j+jstart)
        enddo
      enddo
      ...
      do i=1,ncorner
        do j=1,ncorner
          xmatsub(i+ncorner,j) = xmat2copy(i+istart,j+jstart)
        enddo
      enddo
      ...
      do i=1,ncorner
        do j=1,ncorner
          xmatsub(i+ncorner,j+ncorner) = xmat2copy(i+istart,j+jstart)
        enddo
      enddo
