...
C$        t1=omp_get_wtime()
     ...
     $     ifempty,minlevel,maxlevel,z0targ,ntarg,iztarg)
        t2=second()
C$        t2=omp_get_wtime()
ccc        call prin2('time in d2tallbem=*',t2-t1,1)
c
...
c
ccc        call prinf('lused: ccenters(k)=*', (lcenters+lcorners)/1000,1)
ccc        call prinf('lused(k)=*', (iwlists)/1000,1)
...
C$        t1=omp_get_wtime()
     ...
     1        w(iwlists),lwlists,lused)
        t2=second()
C$        t2=omp_get_wtime()
ccc        call prin2('time in d2tlsts=*',t2-t1,1)
c
        lused777=lused+iwlists
ccc        call prinf('after d2tlsts, ier=*',ier,1)
ccc        call prinf('lused(k)=*', (lused777)/1000,1)
c
