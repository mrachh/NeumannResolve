...
\end{align}
with precision $\epsilon.$ Once these nodes and weights have been generated we proceed by enforcing equality of (\ref{eqn:numpreint}) at the discretization nodes, which yields the system of equations
\begin{align}
...
\begin{align}
\left|\int_0^L K(\gamma(s_i),\gamma(t))\, \sigma(t)\,{\rm d}t- \sum_{j=1}^N W_{ij} \sigma_j\, \sqrt{w_j} \right| < \epsilon,
\end{align}
resulting in the linear system
\begin{align}
  -\pi \sigma_i +  \sum_{j=1}^N W_{ij} \sigma_j \sqrt{w_i w_j}=f(s_i) \sqrt{w_i},\quad i,j=1,\dots,N. 
\end{align}
