...
\qb
We have included a remark to point out that the conditioning for the solution of Neumann problem is inherited from its corresponding Dirichlet discretization. (see remark)
...
\qb
The solution of Laplace's equation in complicated domains is a classical problem in computational physics and has a large array of applications such as electrostatics, fluid flow, diffusion, and magnetohydrodynamics to name a few.  The use of theorems, lemmas, propositions has been mainly to summarize existing work based which serves as a foundation for our method. Here are a few other examples of papers in JCP which we believe are written in a similar style.
...
The unified transform method while handles the solution of Laplace's equation on simple polygons, it does not scale for larger problems like the ones presented in the paper. The "broken wheel" region for which $22,240$ nodes have been used has $107$
corners and edges, thereby just requiring approximately $200$ nodes per edge for discretization. Moreover, our method has been coupled to FMMs thereby able to solve this problems in $O(N)$ CPU time, where $N$ is the number of degrees of freedom required to discretize the problem. We believe that Fokas' method wouldn't be competitive as compared to our method (105 iterations in 15 secs to 13 digits of accuracy and plotting the solution at 250000 points in 6.5 secs on a single CPU core).
Moreover, our method also extends to discretizations of the Helmholtz problem (for any complex wave number in the first quadrant) and we are currently in the process of describing the discretization procedure for both Dirichlet and Neumann boundary value problems. 
\qa
