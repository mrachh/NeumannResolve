...
\end{align}
By a standard abuse of notation we use $\cS$ and $\cD$ to denote both the operators acting on $L^2(\Gamma)$ as well as the corresponding operators acting on $L^2([0,L]),$ the latter being obtained from the former by composition with the parameterization.}


The following theorems {\color{blue}(see for example \cite{kress1989linear})} describe the behavior of the single and double layer potentials in the vicinity of the boundary curve $\Gamma.$
...
\upd{
It is well known that $\bnu \cdot \nabla \cS = \cD^{T}$ where $^T$ denotes the adjoint with respect to
the inner product, 
\beqn
\langle f,g \rangle = \int_{0}^{L} f(\gamma(t)) g(\gamma(t)) {\rm d}t\,,
\eeqn
and $\gamma:[0,L] \to \Gamma$ is a counterclockwise arclength parametrization of $\Gamma.$  
...
\end{align}
where $k(s,t)$ is defined in~\cref{eq:def_kerst}.


\subsection{Reduction of boundary value problems}
In this section we describe the conversion of the Laplace boundary value problems (interior Dirichlet, exterior  Dirichlet, interior Neumann, and exterior Neumann) to second-kind integral equations. \upd{These results are classical and we present them below for completeness (see~\cite{kress1989linear}, for example).}

\begin{enumerate}
\item Interior Dirichlet problem: \\ 
...
\label{bie_intd}
\end{align}
for all $s \in [0,L).$ Moreover, the solution to the interior Dirichlet problem with boundary data $f$ is given by $u(\by) = \cD[\sigma](\by)$ for all $\by \in \Omega.$

\item Exterior Dirichlet problem:\\ 
...
for all $s \in [0,L].$ 
Moreover, the solution to the exterior Dirichlet problem with boundary data $f$ is given by $u(\by) = \cD[\sigma](\by) + \int_{0}^{L} \sigma(t) dt$
...
\upd{
\begin{remark}
We note that the smoothness of the kernel $k$ for well-separated portions of the domain guarantees that the problem of determining the singularities arising in the vicinity of a corner for a polygonal domain can be reduced to the analysis of the wedge case, see the discussion in \cite{hoskins2019numerical,serkhrachh} for example.
\end{remark}
}
